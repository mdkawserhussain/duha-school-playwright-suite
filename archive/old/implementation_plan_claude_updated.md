...
In our project, we will adapt this functionality to be **fully automated** by feeding the scraped, clean JSON databases (extracted from the portal by Playwright) into a TypeScript-based WhatsApp message compiler, along with direct Telegram push notifications.

### Adaptation architecture:

1. **New Module — `src/reporters/whatsappReporter.ts` & `src/reporters/telegramNotifier.ts`:**
   - **WhatsApp Reporter**: Accepts parsed employee attendance and accounts receivable records, compiles templates, and generates `output/WhatsApp-Links-Dashboard.html` with two tabs (Staff Salary Slips / Parent Dues Reminders). Gate behind `GENERATE_WHATSAPP_DASHBOARD=true` in `.env`.
   - **Telegram Notifier**: A post-run notifier script. When execution finishes, compiles metadata metrics (Run duration, Total Due Students, Outstanding Balance, Attendance Anomalies) and dispatches a markdown message to the Telegram API:
     ```bash
...
2. **Integration into main orchestrator:**
   Expose options in `.env` (e.g., `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `ENABLE_TELEGRAM_NOTIFICATIONS=true`). When enabled, the orchestrator (`src/main.ts`) will invoke the notifiers immediately after saving extraction reports.
...
- **API Interception/Request**: For each student identified as "due" in the primary accounts receivable list, make an direct API call:
  - **Endpoint**: `POST https://duhais.eduexpert24.com/site/fee/student-payment-report/get-site-single-student-payment-summary`
  - **Payload**:
    ```json
    {
      "user_name": "2622900113", // Student's unique user name (Roll/ID)
      "academic_year_id": 19,
      "active_status": 1
    }
    ```
...
| 2 | **Add output directories to .gitignore** — ignore all JSON, XLSX, HTML, and log files generated in output/ | [.gitignore](file:///home/ticktick/Desktop/playwright/.gitignore) | Security / Compliance |
...
#### Month 1: Rich Visualization & Reporting
*Theme: Elevate raw scraped data into visual insights and instant notifications.*
- Implement **Standalone HTML Summary Dashboard** with Chart.js charts (Dues by class, collection rate, trend by month, top defaulters).
- Implement **Telegram Notification Pipeline** to dispatch summary metrics after completed runs.
- Implement **Data Change Detection Diff Engine** (`diffSnapshot`) comparing daily JSON snapshots to identify new defaulters, cleared dues, and amount updates.
- Write unit tests for `diffEngine.ts` and core filters.

#### Month 2: Financial Deep-Dive APIs & Scheduling
*Theme: Gather payment details and automate scheduled headless pipelines.*
- Implement **Student-wise Payment Ledger Extraction** by POSTing to the `/get-site-single-student-payment-summary` endpoint for each due student; append to the Excel workbook.
- Implement **Class-wise Waiver Tracking** using the `/get-site-class-base-waiver-list` endpoint; add to the main accounts sheet.
- Integrate **Nightly Scheduler (`node-cron`)** set to run daily at 8:30 AM.
- Integrate **Google Drive & Google Sheets Sync** to upload Excel sheets upon completion.
- Add command-line filters (e.g., `--min-due=5000`) and OS notifications via `node-notifier`.

#### Month 3: Anti-Bot, Security & Packaging
*Theme: Build resilience, stealth, and packaging for distribution.*
- Integrate **Human-like Interaction (`ghost-cursor`)** for dropdown clicks and login inputs to bypass WAF blockades.
- Enable TypeScript strict mode and declare strong interfaces for all incoming JSON shapes.
- Build a lightweight **Tauri or Electron Desktop App** wrapping the local dashboard and scheduling client.
- Add uptime heartbeats (Better Stack/Cronitor) to monitor cron health.
- Decouple extraction layer from reporting generators.
...
1. **Multi-School Commercialization `[Revenue bet]`**
   Validate that other schools on the eduexpert24 platform have identical portal structures. If so, package as a SaaS product sold to other schools on the platform with automated daily extractions, visual dashboards, and Telegram/WhatsApp notification hubs.

2. **Longitudinal SQL Ledger `[Moat bet]`**
   Store every daily run in a local SQLite or Lowdb database. Build multi-month attendance histories and payment trends. This historical data acts as a lock-in mechanism.

3. **Vite/Next.js Financial Web Portal `[UX bet]`**
...
1. **Multi-browser support (Firefox, WebKit)**
   *Why:* The target portal runs reliably on Chromium. Emulating WebKit or Firefox adds testing cycles for zero gain.
2. **In-portal write operations (payment processing, attendance marking)**
   *Why:* Keeps the script read-only. Automating financial edits on the portal carries extreme legal and financial risks.
3. **Full SaaS Auth System early on**
   *Why:* A local database and HTML dashboard meet the administrators' operational needs without hosting cost and web app complexity.

---

## 4. ONE-PARAGRAPH COMMERCIAL PITCH

"The School Management Portal Automation Suite transforms the daily grind of manual school data exports into a lights-out, zero-touch pipeline. In under two minutes, it securely authenticates into your eduexpert24 portal, captures complete staff attendance records via direct API interception, extracts student accounts receivable across every class and shift, filters to outstanding balances, and produces publication-ready Excel ledgers sorted by class and dues amount. It automatically queries individual payment ledgers and concessions for due students, processes day-over-day financial diffs, sends instant summary reports to Telegram, and compiles a stunning, self-contained HTML visual dashboard. Built with credential-safe architecture, automatic PII rotation, and human-like browser movements, it is the first tool purpose-built for the operational reality of schools running on eduexpert24 — and every school on the platform can be onboarded by changing a single URL."
