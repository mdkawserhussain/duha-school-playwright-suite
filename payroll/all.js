...
// Map EVERYONE from config.staff (Source of Truth) and sort alphabetically
...
// Sort payroll alphabetically by name
...
function buildMonthlyAll() {
  const colW = [500, 2200,530,530,530,530,430,530,530,530,680,680,680,820,680,680,900,700,900,1200,700];
  const totalW = colW.reduce((a,b)=>a+b,0);
  const headers = ["SL", "Teachers' Name","W. Days","P. Days","Casual Leave","A. Days","Late","Over Time","Increment","Bonus","PF Deductions","PF Return","Per Day Salary","Basic Salary","Allowances","Tiffin Allow.","Total Salary","Total Ded.","Net Payable","Details","Markings"];
  ...
  const dataRows = payroll.map((p, idx) => {
    ...
    return new TableRow({
      children: [String(idx + 1), p.name, empWorkingDays, p.pdays, p.leave, p.absent, p.late, p.ot || 0, p.increment || 0, p.bonus || 0, p.pfDeduction || 0, p.pfReturn || 0, p.perDay.toLocaleString(), p.basic.toLocaleString(), p.genAllow.toLocaleString(), p.tiffin.toLocaleString(), p.gross.toLocaleString(), p.totalDed.toLocaleString(), p.net.toLocaleString(), p.markings, p.excNote || ""].map((v, i) => new TableCell({
        ...
        children:[new Paragraph({alignment: i<=1?AlignmentType.LEFT:AlignmentType.CENTER, children:[new TextRun({text:String(v),size:15,bold:i<=1,font:"Arial"})]})]
      }))
    });
  });
...
function buildMonthlyAllFormatted() {
  const colW = [450, 2243, 462, 463, 555, 463, 425, 457, 713, 536, 782, 869, 534, 679, 787, 521, 679, 850, 960, 2419];
  const totalW = colW.reduce((a, b) => a + b, 0);
  const headers = [
    "SL", "Teachers' Name", "W. Days", "P. Days", "Casual Leave", "A. Days", "Late", "Over Time", 
  ...
  for (let i = 0; i < 20; i++) {
    ...
    totalRowCells.push(new TableCell({
      ...
      verticalAlign: VerticalAlign.CENTER,
      children: [new Paragraph({
        alignment: i === 17 ? AlignmentType.RIGHT : AlignmentType.CENTER,
        spacing: { before: 0, after: 0 },
...
function buildBankLetter() {
  const bankStaff = payroll.filter(e => e.acct).sort((a, b) => a.name.localeCompare(b.name));
  const cashStaff = payroll.filter(e => !e.acct).sort((a, b) => a.name.localeCompare(b.name));
...
async function main() {
  ...
  const bankStaffCSV = payroll.filter(e => e.acct).sort((a, b) => a.name.localeCompare(b.name));
