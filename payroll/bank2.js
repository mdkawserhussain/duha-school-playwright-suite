...
function parseMonthlyAll(filePath) {
  ...
  rows.forEach(row => {
    ...
    // Monthly All header has 21 columns. Data rows should have at least that.
    ...
    const pdays = parseInt(getCellText(cells[3]).replace(/,/g, '') || 0);
    const absent = parseInt(getCellText(cells[5]).replace(/,/g, '') || 0);
    const netStr = getCellText(cells[18]).replace(/,/g, '');
