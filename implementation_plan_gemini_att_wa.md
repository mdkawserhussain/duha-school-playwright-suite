...
1. **Setup Response Interceptor:**
   ...
   ```typescript
   const apiEndpoint = '**/site/employee/attendance/report/employee-date-wise-attendance-list';
...
## Proposed Changes: WhatsApp Notification Generator

The script `wa2.js` parses staff payroll and attendance data from Microsoft Word files and generates WhatsApp message notifications and an interactive links dashboard. 
...
1. **New Module - `src/utils/whatsappGenerator.ts`:**
   ...
   - Configuration profiles (from a local `config.json` containing staff phone numbers and templates).
...
2. **Message Compilers:**
   ...
   - **Outstanding Dues Compiler:** Translates a student accounts receivable record (name, ID, class, total due, parent phone number) into a parent-facing WhatsApp reminder.
     ...
     *OUTSTANDING DUES NOTICE*
     ...
     Dear Parent,
     This is to notify you that child *[Student Name]* (ID: [ID], Class: [Class]) has outstanding balances totaling *BDT [Amount]*.
...
3. **Output Files Generation:**
   - Generate `output/wa-data-monthly.js` dynamically using ES6 templates.
...
- **Immediate pipeline flow:** Scraping $\rightarrow$ Report Compiling $\rightarrow$ WhatsApp Sending becomes a single seamless workflow.
